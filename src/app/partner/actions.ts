...
export async function fetchPartnerDashboard(): Promise<ActionResult<PartnerDashboardData>> {
...
export async function fetchPartnerProducts(): Promise<ActionResult<PartnerProductDetail[]>> {
  const partnerId = await getAuthenticatedPartnerId();
  if (!partnerId) return { success: false, error: "파트너 인증이 필요합니다." };
  try {
...
export async function fetchPartnerOrders(): Promise<ActionResult<PartnerOrderItem[]>> {
  const partnerId = await getAuthenticatedPartnerId();
  if (!partnerId) return { success: false, error: "파트너 인증이 필요합니다." };
  try {
